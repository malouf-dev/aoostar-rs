...
From the repository root on the host the LCD is plugged into:

```shell
docker compose up -d --build
```

That's it. The image builds from source (first build takes a few minutes),
...
rebuild, or mount your own file over
`/app/cfg/sensor-mapping/docker.tmpl`.
